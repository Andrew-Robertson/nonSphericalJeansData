...
filename_options = ['snap', 'group', 'group_snip', 'sub', 'particles', 'snip', 'particles_snip', 'sub_snip']

class read_eagle_file():
	def __init__( self, model_dir, filename, snapnum, suppress=True, gadgetunits=False, subsample_factor=1):
		"""initialise the class by providing a directory, a file type and a snapshot number.
		Also set the units used to return the data. You can subsample particles if necessary.""" 
		...
		if not suppress: print("trying to read data from ", self.fname)
	...
	def get_full_dir(self, model_dir, filename, snapnum):
		...
		if filename == 'snap':
			...
			#otherwise use old format
				...
				#Special case for RichardIsolated simulation.
				if 'RichardIsolated' in model_dir  or 'MicheleIsolated' in model_dir:
					fname = dirpath + "/snapshot_%.3i/snapshot_%.3i."%(snapnum, snapnum)
				elif 'GigaGalaxy' in model_dir:
					fname = dirpath + "/snapshot_%.3i."%(snapnum)

				elif 'BAHAMAS' in model_dir:
					fname = dirpath + "/snapshot_%.3i/snap_%.3i."%(snapnum, snapnum)
				else:
			...
			try:
				...
				#Exception to ignore .tar files.
				dir = [i for i in fnmatch.filter( os.listdir(dirpath), dirname) if '.tar' not in i][0]
				string = dir.rsplit('_')
				fname = dirpath + dir + "/eagle_subfind_snip_tab_%.3i_%s." % (snapnum, string[-1])
			except:
				print('[read_eagle_files_mpi.py] Could not load sub snip.')
			...
			try:
				...
				print('[read_eagle_files_mpi.py] Could not load particle snip.')
		else:
			try:
		...
		if filename == 'snap' or filename=='snip' or filename == 'particles':
		   self.DM_Particle_Mass = f['Header'].attrs['MassTable'][1] # Load DM particle mass.
	...
	def read_file_attributes(self, f):
		...
		try:	
			...
			self.gamma = 0; self.solar_mass = 0; self.boltzmann = 0; self.protonmass = 0; self.sec_per_year =0
		...
		try:
			...
			self.solarabundance = 0; self.solarabundance_oxygen=0; self.solarabundance_iron=0
	...
	def read_data_array(self, var, gadgetunits=False, suppress=False, interval=1, region=None, frames=None, requiredlength=1e9, extra_basename=None):

		#raise Exception("Must use region in MPI mode!")
		...
		string = var.rsplit('/')
		try:
			parttype = N.int32(string[0][-1])  #...select the integer bit...
		except:
